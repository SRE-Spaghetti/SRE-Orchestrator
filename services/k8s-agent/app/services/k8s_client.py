...
def initialize_kubernetes_client():
    ...
    try:
        ...
        logger.info(
            f"Successfully listed {len(pods.items)} pods in namespace {namespace}."
        )
        return core_v1_api
    except config.ConfigException:
        logger.error(
            "Could not load in-cluster Kubernetes config. Are we running in a cluster?"
        )
        return None
        ...
        logger.error(
            f"An unexpected error occurred during Kubernetes client initialization: {e}"
        )
        return None
        ...
        if pod.status.container_statuses:
            for cs in pod.status.container_statuses:
                ...
                container_statuses.append(
                    ContainerStatus(name=cs.name, state=state, ready=cs.ready)
                )
...
def get_pod_logs(
    ...
    try:
        ...
        if e.status == 404:
            logger.info(
                f"Pod {name} or container {container} not found in namespace {namespace}."
            )
            return None
