...
class HttpServerConfig(BaseModel):
    ...
    transport: str = Field(
        "streamable_http", description="Transport type (must be 'streamable_http')"
    )
    headers: Optional[Dict[str, str]] = Field(
        None, description="HTTP headers (e.g., for authentication)"
    )
...
class MCPServerConfig(BaseModel):
    ...
    headers: Optional[Dict[str, str]] = Field(
        None, description="Headers for HTTP transport"
    )
